					...
					<?php if( empty($EM_Event->group_id) ): ?>
						...
						<p>
							<select name="group_id">
								<option value="<?php echo $BP_Group->id; ?>">Keine AG-Veranstaltung</option>
							<?php
							...
							<br />
							<?php _e ( 'Wenn es eine Veranstaltung einer Arbeitsgruppe von dir ist, dann wähle deine AG aus, damit die Gruppenmitglieder die Veranstaltung mitgestalten können. Um eine AG auswählen zu können, muss man zuvor der Arbeitsgruppe beigetreten sein: <a hreF="/groups">Arbeitsgruppen</a>', 'dbem' )?>
						</p>
						<?php endif; ?>
					<?php endif; ?>
				</div>
			...
			<div class="right">
				<div class="inside event-form-where">
						...
						<div id="em-location-data" style="padding-right:20px; vertical-align:top;">
							...
							<p><?php _e ( 'Wähle ein Standort', 'dbem' )?> <?php if(current_user_can('edit_locations')){ echo sprintf(__('oder <a href="%s">erstelle einen neuen Standort</a>','dbem'),$location_link); } ?></p>

							<?php if ( get_option ( 'dbem_gmap_is_active' ) ) : ?>
							<div id='em-map-404' style='width: 100%; vertical-align:middle; text-align: center;'>
								<p><em><?php _e ( 'Location not found', 'dbem' ); ?></em></p>
							</div>
							<div id='em-map' style='width: 106%; height: 300px; display: none;'></div>
							<?php endif; ?>
						</div>
				</div>
			</div>
			...
			<div class="left">
				<div class="inside event-form-image">
					<h4>Flyer</h4>
						...
						<?php endif; ?>
						<br /><br />
						<label for='event_image'>Flyer auswählen:</label><br /><input id='event-image' name='event_image' id='event_image' type='file' size='20' />
						<br />
						<label for='event_image_delete'><?php _e('Flyer löschen?', 'dbem') ?></label> <input id='event-image-delete' name='event_image_delete' id='event_image_delete' type='checkbox' value='1' />
				</div>
			</div>
			<div class="right">
				<div class="inside event-form-details">
					...
					<div>
						<textarea name="content" rows="40" style="width:100%"><?php echo $EM_Event->notes ?></textarea>
						<br />
				...
				<div class="inside event-form-categories">
					<h4>Kategorie</h4>
